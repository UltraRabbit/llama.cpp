...
ggml_tensor * llama_model_base::create_tensor(llama_model_loader & ml, const LLM_TN_IMPL & tn, const std::initializer_list<int64_t> & ne, int flags) {
    ...
    if (tn.bid != -1) { 
        if ((size_t) tn.bid >= pimpl->dev_layer.size()) { 
            LLAMA_LOG_WARN("%s: tensor '%s' has block index %d but model only has %zu layers (n_layer_all=%d) - falling back to CPU\n", 
                __func__, tn.str().c_str(), tn.bid, pimpl->dev_layer.size(), hparams.n_layer_all); 
            buft_list_layer = &pimpl->cpu_buft_list; 
        } else { 
            buft_list_layer = pimpl->dev_layer[tn.bid].buft_list; 
        } 
    }
